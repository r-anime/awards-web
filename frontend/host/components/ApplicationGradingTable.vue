			...
			<table v-if="selectedQuestionID !== '-1'" class="table is-hoverable is-fullwidth">
				<tbody>
					...
					<tr v-for="answer in filteredAnswers" :key="answer.id">
						<td><router-link :to="`/host/applications/${application.id}/applicant/${answer.applicant.id}`">{{showNames ? answer.applicant.user.reddit : answer.applicant.id}}</router-link></td>
